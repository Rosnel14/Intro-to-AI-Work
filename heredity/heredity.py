...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        # this is if no parents are found, so I calculate unconditional probability
        if mother is None and father is None:
            ...
            probability *= (
                ...
                # this is for getting the gene from one parent but not the other
                passing[mother] * (1 - passing[father]) + (1 - passing[mother]) * passing[father] if genes == 1 else

                # not getting it from any of the parents
                (1 - passing[mother]) * (1 - passing[father])
            )

        # probability of a given/not given trait
